...
const numbOdd = randomNumbers => randomNumbers % 2 === 1;
const numbResult = randomNumbers.filter(numbOdd);
...
const getNumber = (accumulator, crazyNumbers) => crazyNumbers < 501 ? ++accumulator : accumulator

const numberLessResult = crazyNumbers.reduce(getNumber, 0)
console.log(numberLessResult)
...
const numbersQuad = numbers.map(item => item ** 2)
console.log(numbersQuad)
...
const getMovies = tarantinoMovies.filter( ({ release }) => release < 2000)
...
const nameTvShow = tvShows.map(({ name }) => name)
console.log(nameTvShow)
